(function () {
  ...
  Date.now || (Date.now = function () {
    ...
    t.snowfall = function (e, i) {
      ...
      return "object" == _typeof(e) || void 0 == e ? this.each(function (i) {
  ...
  (function () {
    ...
    var version_modss = '3.0',
        ...
        vip = false,
        user_id = '',
        uid = '490a2c8bc0ca38f46539b76a7b887634',
        IP,
        logged = false;
    var Modss = {
      init: function init() {
      ...
      radio: function radio() {
        ...
        if (Lampa.Storage.get('mods_radio')) $('body').find('.menu .menu__list').eq(0).append(menu_item);else $('body').find('[data-action="Radio_n"]').remove();
      },
      ...
      online: function online(back) {
        ...
        this.btn = btn; //	if (Lampa.Storage.field('online_but_first')) Lampa.Storage.set('full_btn_priority', Lampa.Utils.hash(btn.clone().removeClass('focus').prop('outerHTML')));
        ...
        if (!back && Lampa.Storage.field('mods_onl')) {
          ...
          btn.unbind('hover:enter hover.click').on('hover:enter hover.click', function () {
            ...
            if (_this.is_continue && Lampa.Storage.field('online_continued')) shows(_this.last_s);else openOnline();
          });
        }
      },
      collections: function collections() {
        ...
        if (Lampa.Storage.get('mods_collection')) $('body').find('.menu .menu__list li:eq(3)').after(menu_item);else $('body').find('[data-action="collection"]').remove();
        menu_item.on('hover:enter', function () {
          var item = [{
      ...
      serialInfo: function serialInfo(card) {
        if (Lampa.Storage.field('mods_serial_info') && card.source == 'tmdb' && card.seasons && card.last_episode_to_air) {
          ...
          if (!$('.card--new_seria', Lampa.Activity.active().activity.render()).length) {
            if (window.innerWidth > 585) $('.full-start__poster,.full-start-new__poster', Lampa.Activity.active().activity.render()).append("<div class='card--new_seria' style='right: -0.6em;position: absolute;background: #168FDF;color: #fff;bottom:.6em;padding: 0.4em 0.4em;font-size: 1.2em;-webkit-border-radius: 0.3em;-moz-border-radius: 0.3em;border-radius: 0.3em;'>" + Lampa.Lang.translate(new_ser) + "</div>");else {
              if ($('.card--new_seria', Lampa.Activity.active().activity.render()).length) $('.full-start__tags', Lampa.Activity.active().activity.render()).append('<div class="full-start__tag card--new_seria"><img src="./img/icons/menu/movie.svg" /> <div>' + Lampa.Lang.translate(new_ser) + '</div></div>');else $('.full-start-new__details', Lampa.Activity.active().activity.render()).append('<span class="full-start-new__split">●</span><div class="card--new_seria"><div>' + Lampa.Lang.translate(new_ser) + '</div></div>');
            }
          }
        }
      },
      rating_kp_imdb: function rating_kp_imdb(card) {
        return new Promise(function (resolve, reject) {
          ...
          var year = parseInt((relise + '').slice(0, 4)); //	if (Lampa.Storage.field('mods_rating') && $('.rate--kp', Lampa.Activity.active().activity.render()).hasClass('hide') && !$('.wait_rating', Lampa.Activity.active().activity.render()).length)

          if (['filmix', 'pub'].indexOf(card.source) == -1 && Lampa.Storage.field('mods_rating')) $('.info__rate', Lampa.Activity.active().activity.render()).after('<div style="width:2em;margin-top:1em;margin-right:1em" class="wait_rating"><div class="broadcast__scan"><div></div></div><div>');
          Pub.network.clear();
      ...
      check: function check(name, call) {
        ...
        if (name && !call) check();else if (call && name && !Modss.jack[name].check) check(true);else {
      ...
      showModal: function showModal(text, onselect) {
        Lampa.Modal.open({
          title: '',
          align: 'center',
          zIndex: 300,
    ...
    var Filmix = {
      ...
      showStatus: function showStatus(ch) {
        ...
        if (status.login) {
          ...
          if (status.is_pro || status.is_pro_plus) info = foto + ' - <b>' + (status.is_pro ? 'PRO' : 'PRO_PLUS') + '</b> ' + Lampa.Lang.translate('filter_rating_to') + ' - ' + status.pro_date + '</span>';else info = foto + ' - <b>NO PRO</b> - MAX 720p</span>';
        }

        if (ch) $('.settings-param__descr', ch).html(info);else $('.settings-param__descr:eq(0)').html(info);
      },
      checkPro: function checkPro(token, call, err) {
    ...
    var ForkTV = {
      ...
      create__id: function create__id() {
        var randomNum = Math.floor(Math.random() * 900000) + 100000;
        return randomNum;
      },
      create_dev_id: function create_dev_id() {
      ...
      cats_fork: function cats_fork(json) {
        var item = [];
      ...
      parse: function parse() {
        ForkTV.check(ForkTV.url, function (json) {
          json = json.channels;
          if (json.length === 1) ForkTV.checkAdd();else {
            ForkTV.but_add();
      ...
      check_forktv: function check_forktv(itm, ar) {
      ...
      check: function check(url, call, ar) {
        this.network.clear();
        this.network.timeout(8000);
    ...
    var Pub = {
      ...
      refreshTok: function refreshTok() {
      ...
      userInfo: function userInfo(itm, ur) {
        var status = $('.settings-param__status', itm).removeClass('active error wait').addClass('wait');
        if (!Pub.token) status.removeClass('wait').addClass('error');else {
      ...
      info_device: function info_device() {
      ...
      delete_device: function delete_device(call) {
    ...
    function component(object) {
        ...
        filter.onSelect = function (type, a, b) {
          if (type == 'filter') {
            ...
            if (Lampa.Arrays.getKeys(sources).indexOf(a.source) !== -1) {
              if (!Lampa.Storage.get('pro_pub', false) && a.source == 'pub') return Lampa.Noty.show('Доступно для VIP пользователей<br>Через бот @modssmy_bot');
      ...
      this.proxy = function (name) {
        ...
        if ((need == 'on' || main) && name == 'kinobase' && need_url.length == 0) return myprox;else if (need == 'on' && need_url.length >= 0 && prox !== '') proxy = prox;else if (need == 'url' || need == 'on' && need_url.length > 0) proxy = need_url;else if (prox && need == 'on') proxy = prox; //else if (main && need == 'on') proxy = myprox;
      ...
      this.find = function () {
        ...
        var vcdn_search = function vcdn_search() {
          ...
          if (object.movie.kinopoisk_id || object.movie.kinopoisk_ID) url = Lampa.Utils.addUrlComponent(url, 'kinopoisk_id=' + encodeURIComponent(object.movie.kinopoisk_id || object.movie.kinopoisk_ID));else url = url_by_title;
        ...
        var kp_search = function kp_search() {
          ...
          network.timeout(1000 * 15);
          network.silent(url, function (json) {
            if (json.items && json.items.length) display(json.items);else if (json.films && json.films.length) display(json.films);else display([]);
          }, function (a, c) {
            vcdn_search();
          });
        };
        ...
        var letgo = function letgo(imdb_id) {
          if (['videocdn', 'videoapi'].indexOf(balanser) >= 0) vcdn_search();else if (source.searchByKinopoisk) kp_search();else if (imdb_id && source.searchByImdbID) {
            _this2.extendChoice();
      ...
      this.similars = function (json) {
        ...
        json.forEach(function (elem) {
          ...
          if (elem.type) info.push(elem.type == 'serial' || elem.type == 'MINI_SERIES' ? 'Cериал' + (count_s && ' - ' + count_s + ' из них ' + count_eps) : elem.type == 'TV_SHOW' ? 'Тв-Шоу' : elem.type == ('movie'  ) ? 'Фильм' : elem.type);
      ...
      this.draw = function (items) {
        ...
        this.getEpisodes(object.movie.source == 'pub' || balanser == 'pub' || object.movie.original_language !== 'ja' || object.movie.number_of_seasons >= choice.seasons ? items[0].season : 1, function (episodes) {
          ...
          items.forEach(function (element, index) {
            ...
            serial && episodes.length && !params.similars ? episodes.find(function (e, i) {
            ...
            html.on('hover:enter', function () {
              ...
              if (['pub', 'collaps'].indexOf(balanser) >= 0) {
                $('.voices').remove();
                $('.explorer-card__descr').hide().after('<div class="voices"></div>'); //$('.explorer-card__descr').html(Lampa.Lang.translate('<b>#{torrent_parser_voice}:</b><br>' +element.voice));
      ...
      this.start = function () {
        ...
        Lampa.Controller.add('content', {
          ...
          right: function right() {
            if (Navigator.canmove('right')) Navigator.move('right');else if (object.movie.number_of_seasons) filter.show(Lampa.Lang.translate('title_filter'), 'filter');else filter.show(Lampa.Lang.translate('modss_balanser'), 'sort');
          },
          left: function left() {
    ...
    function forktv(object) {
      ...
      this.create = function () {
        ...
        this.activity.loader(true);
        if (object.submenu) _this.build(object.url);else {
      ...
      this.stream = function (data, title, youtube, subs, element, view) {
        ...
        if (data.indexOf('getstream') == -1 && (data.indexOf('rgfoot') > -1 || data.indexOf('torrstream') > -1 || data.indexOf('torrent') > -1)) {
          ...
          if (Lampa.Platform.is('android')) Lampa.Android.openYoutube(id);else _this.YouTube(id);
        }
      };
              ...
              if (element.title.match('Описание|Трейлер') == null) {
                if (element.template && element.template.indexOf('film.') > -1 || element.logo_30x30 && element.logo_30x30.match('mediafil') || element.logo_30x30 && element.logo_30x30.match('folder') && element.playlist_url && element.playlist_url.indexOf('torrstream?magnet') > -1) {
                  ...
                  if (!element.search_on) {
                    ...
                    !element.ident && element.description && $($(element.description).children()[1]) ? $($(element.description).children()[1]).text().slice(0, 130) || $($(element.description).children()[0]).parent().text().slice(0, 130) : '';
                  ...
                  image && image.indexOf('yandex') > -1 ? 'https://cors.eu.org/' + image : image && image.indexOf('svg') > -1 ? image : image;
                  img.src = image;
                ...
                card.on('hover:focus hover:touch', function () {
                  if (this.className.indexOf('card--category') > -1) {
                    if (Lampa.Helper) Lampa.Helper.show('online_file', 'Удерживайте клавишу (ОК) для просмотра описания', card); //Lampa.Background.immediately(image);
                  }
                  ...
                  if (stream || data.channels.length > 0) {
                    if (element.event || stream && stream.match(/youtube|stream\?|mp4|mkv|m3u8/i)) {
                      ...
                      var _goto = function _goto2() {
                        var title =
                        /*stream == 'submenu' ? element.submenu && element.submenu[0].title : */
                        element.details && element.details.title ? element.details.title : element.title && element.title.indexOf('l-count') > -1 ? element.title.split(' ').shift() : element.details && element.details.name ? element.details.name : element.title; //console.log (element.submenu)
                      ...
                      if (element.title == '18+' && Lampa.Storage.get('mods_password')) {
                        Lampa.Input.edit({
                          ...
                          if (Lampa.Storage.field('mods_password') == t) _goto();else {
    ...
    function collection(object) {
      ...
      this.create = function () {
        ...
        if (object.sourc == 'my_coll') {
          ...
          if (object.card && isNaN(object.id)) url = object.id;else if (object.sourc == 'pub') {
            if (object.search) url = object.url + '?title=' + object.search + '&sort=views-&access_token=' + Pub.token;else url = object.url + '?sort=' + (object.sort ? object.sort : 'views-') + '&access_token=' + Pub.token;
          } else if (object.sourc == 'rezka') url = object.url + '?filter=last';else url = object.url;
      ...
      this.next = function (page) {
        ...
        if (object.sourc == 'pub') url = object.url + '?page=' + object.page + '&sort=' + (object.sort ? object.sort : 'views-') + '&access_token=' + Pub.token;else url = page.replace(/(\d+)\/\?filter/, object.page + '/?filter');
        network.silent(cors + url, function (result) {
          ...
          if (data.card.length) waitload = false; //Lampa.Controller.toggle('content');
        ...
        datas.forEach(function (element) {
          ...
          card.onFocus = function (target, card_data) {
            ...
            if (!Lampa.Platform.tv() || !Lampa.Storage.field('light_version')) {
              Math.ceil(items.length / 7) - 1; //if (Math.ceil(items.indexOf(card) / 7) >= maxrow) _this1.next(data.page);
            }
          };
      ...
      this.build = function (data) {
        ...
        if (data.card.length || Lampa.Arrays.getKeys(data.card).length) {
          ...
          this.append(data); //	if (Lampa.Platform.tv() && Lampa.Storage.field('light_version')) this.more(data);
      ...
      this.more = function (data) {
        var _this = this; //	var more = $('<div class="category-full__more selector"><span>' + Lampa.Lang.translate('show_more') + '</span></div>');
        ...
        var url;
        if (object.sourc == 'pub') url = object.url + '?page=' + data.page + '&sort=' + (object.sort ? object.sort : 'views-') + '&access_token=' + Pub.token;else url = data.page;
        network.silent(cors + url, function (result) {
          ...
          next.page++;
          if (card.card.length == 0) more.remove();else Lampa.Activity.push(next);
        }, function (a, c) {
          Lampa.Noty.show(network.errorDecode(a, c));
        }, false, {
          dataType: 'text'
    ...
    function Radio_n(object) {
      ...
      this.start = function () {
        ...
        Lampa.Controller.add('content', {
          ...
          right: function right() {
            if (Navigator.canmove('right')) Navigator.move('right');else _this.showStancia();
          },
          up: function up() {
            if (Navigator.canmove('up')) {
              Navigator.move('up');
          ...
          down: function down() {
            if (Navigator.canmove('down')) Navigator.move('down');else Lampa.Controller.toggle('content');
          },
          back: function back() {
            Lampa.Activity.backward();
          }
    ...
    function startPlugin() {
      ...
      Lampa.Listener.follow('full', function (e) {
        if (e.type == 'complite') {
          ...
          Modss.online();
          Modss.rating_kp_imdb(e.data.movie).then(function (e) {})['catch'](function (e) {
            {
              START_PRELOAD;
            }
          });
      ...
      Lampa.Storage.listener.follow('change', function (e) {//if(e.name == 'jackett_key' || e.name == 'jackett_url') Modss.check(e.value);
      });
      ...
      function add() {
        ...
        $('body').append(Lampa.Template.get('modss_online_css', {}, true)); //	Lampa.Storage.set('guide', '');

        setTimeout(function () {//if (window.innerHeight > 700 && Lampa.Storage.field('guide') == 'undefined') guide();
        }, 3000);
        Lampa.SettingsApi.addComponent({
        ...
        Lampa.SettingsApi.addParam({
          ...
          field: {
            name: ' 💻 Macintosh (rv:109.0) Firefox'
          }
        });
        Lampa.SettingsApi.addParam({
          component: 'settings_modss',
        ...
        Lampa.SettingsApi.addParam({
          ...
          onChange: function onChange(value) {
            if (value == 'true') Modss.collections();else $('body').find('.menu [data-action="collection"]').remove();
          }
        }); //Styles
      ...
      function search$3() {
        ...
        get$6('v1/items/search', mov, function (json) {
          ...
          if (json.items) {
            json.items.forEach(function (element) {
              if (element.type == 'movie') items.push(tocard(element));else itemss.push(tocard(element));
            });
            var movie = {
              results: items,
              page: json.pagination.current,
