...
function init() {
    ...
    Playlist.listener.follow('playlistEnded', (e) => {
        backward()
    })
    ...
    Video.listener.follow('restart', (e) => {
        let params = Video.saveParams()
        let item = work;
        destroy()
        play(item)
...
function play(data) {
    ...
    let lauch = () => {
        preload(data, () => {
            ...
            Video.setParams({track: data.translate?.selectedIdx, sub: data.selectedSubsIdx})
        })
    }

    if (launch_player === 'other') {
        window.open(data.url, '_blank');
    } else if (launch_player == 'lampa') lauch()
