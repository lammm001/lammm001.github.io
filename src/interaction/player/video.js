...
function init() {
    ...
    rewindIcon = html.find('.player-video__rewind-icon')
    rewindIcon[0].addEventListener('animationend', function () {
        rewindIcon.removeClass('animate-in-rewind')
    });
    rewindIconValue = html.find('.player-video__rewind-icon span')[0]
    ...
    forwardIconValue = html.find('.player-video__forward-icon span')[0]
    ...
    if (tracks.length) {
        for (let i = 0; i < tracks.length; i++) {
            if (tracks[i].enabled == true || tracks[i].selected == true) {
                params.track = i
                params.trackName = tracks[i].title
            }
        }
    }
 ...
 */
function play() {
    if (timer.retryPlayOnError) clearTimeout(timer.retryPlayOnError)
    ...
    if (playPromise !== undefined) {
        playPromise.then(function () {
            console.log('Player', 'start plaining')
                ...
                if (e.message && !e.message.includes('request was interrupted by a call to pause')
                    && (video.src === videoUrl || hls)) {
                    timer.retryPlayOnError = setTimeout(function () {
                        listener.send('restart', {})
                        listener.send('reset_continue', {})
                    }, 5000);
                }
            },
...
function rewindOnMultiClick(numClicks) {
    let rewindSeconds = (numClicks - 1) * seekSpeed;
    rewindIconValue.innerHTML = `-${rewindSeconds}  sec`;
    rewindIcon.addClass('animate-in-rewind');
    to(video.currentTime - rewindSeconds);
}

function forwardOnMultiClick(numClicks) {
    let forwardSeconds = (numClicks - 1) * seekSpeed;
    forwardIconValue.innerHTML = `+${forwardSeconds}  sec`;
    forwardIcon.addClass('animate-in-forward');
    to(video.currentTime + forwardSeconds);
}

function handleClick(e) {
    let shouldRewindVideo = isShouldRewindVideo(e);
    if (clickTimer) clearTimeout(clickTimer);
    numClicks++;
    if (numClicks === 1) {
