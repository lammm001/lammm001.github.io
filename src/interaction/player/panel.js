...
function init() {
    ...
    elems = {
        ...
        timelineWrapper: $('.player-panel__timeline-wrapper', html),
        quality: $('.player-panel__quality', html),
        episode: $('.player-panel__next-episode-name', html)
    }

    html.find('.player-panel__fullscreen').toggleClass('hide', Platform.tv() || !Utils.isTouchDevice()
        || !Utils.canFullScreen())
    ...
    elems.timelineWrapper.on('click', (e) => {
        ...
        if (tracks.length) {
            ...
            Select.show({
                ...
                onSelect: (a) => {
                    ...
                    Controller.toggle(enabled.name)
                    setTimeout(function () {
                        listener.send('saveParams', Video.saveParams())
                    }, 1000)
                },
                onBack: () => {
                    Controller.toggle(enabled.name)
...
function visible(status) {
    ...
    timer.toggleVisible = setTimeout(() => {
...
function destroy() {
    ...
    clearTimeout(timer.toggleVisible)
    clearTimeout(timer.hide)
