...
function setSpeed(v) {
    ...
    html.querySelector('#speedtest_num').innerHTML = (v < 1 ? v.toFixed(3) : (v < 10 ? v.toFixed(2) : (v < 100 ? v.toFixed(1) : Math.round(v))))

    let r = speed2deg(v)
    let b = html.querySelector('#speedtest_progress')
    let l = 1256.8

    b.style['stroke-dasharray'] = l * r / 360 + ',' + l
    b.style.stroke = hslToRgb(330 + r, 80, 45)

    html.querySelector('#speedtest_graph').setAttribute('points', graph.map(function (pt) {
        return pt.join(',')
    }).join(' '))
}
...
function testUrl(url) {
    ...
    let errorFn = function (e) {
        html.querySelector('#speedtest_status').html(Lang.translate('network_error'))
    }
...
function testSpeed(url) {
    let context = this
    let status = html.querySelector('#speedtest_status')
    let time
