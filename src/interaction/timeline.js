...
function resetForMovie(title) {
    ...
    for (let seasonNum = 0; seasonNum < 50; seasonNum++) {
        for (let episodeNum = 0; episodeNum < 200; episodeNum++) {
            let hash = Lampa.Utils.hash([seasonNum, episodeNum, title].join(''));
            delete viewed[hash]
        }
    }
...
function update(params) {
    ...
    if (params.duration === 0) { // it's a reset
