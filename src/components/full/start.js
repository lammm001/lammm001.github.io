...
function create(data, params = {}) {
    ...
    this.create = function () {
        ...
        if (data.movie.genres) {
            genres = data.movie.genres.slice(0, 3).map((a) => {
    ...
    this.parsePG = function () {
        ...
        if (data.movie.release_dates && !pg) {
            let find = data.movie.release_dates.results.find(a => a.iso_3166_1 == cd.toUpperCase()
                && a.release_dates.find(a => a.certification))

            if (!find) find = data.movie.release_dates.results.find(a => a.iso_3166_1 == 'US'
                && a.release_dates.find(a => a.certification))

            if (!find) find = data.movie.release_dates.results.find(a => a.release_dates.find(a => a.certification))

            if (find) {
                let release_date = find.release_dates.find(a => a.certification)
                pg = Utils.decodePG(release_date?.certification)
            }
        }
